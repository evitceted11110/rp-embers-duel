...
## 世界內編號字符（2026-08-05 新增）

狀態：設計階段規格，尚未進入實作／建置。完整定義見 `qe-attack-rune-rework.md`「美術差異化需求」附錄之「世界內編號字符規格」，此處僅摘錄供本文件自足查閱。

- 用於場上需要序列標示的物件（例如同時存在多個的核心、影線折返節點）：6×7px 像素徽標，`#211f2a` 底色圓形、`#f2df9b`（守勢金白）1px 描邊，數字為 3×5px 無反鋸齒像素字型。
- 本規格所有卡片上限均為 2（①／②），不設計 ③ 以上樣式。
- 徽標以 billboard 懸浮於物件上方 4px、不隨世界旋轉；武裝時以 2 幀淡入，不可瞬現。
- 即將被下一次觸發消耗的物件，其徽標在 70%↔100% 透明度間以 0.4 秒週期呼吸，其餘維持靜態 70%，用亮度差異取代隱藏觸發順序。
